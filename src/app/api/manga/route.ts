...
export async function GET() {
    ...
    if (cachedManga && Date.now() - lastFetchTime < 3600000) {
        return NextResponse.json({ data: cachedManga });  // Retourne une réponse JSON avec les mangas en cache.
    }
    ...
    try {
        ...
        return NextResponse.json({ data: cachedManga });
    } catch (error) {
