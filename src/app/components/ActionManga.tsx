...
export default function ActionManga() {
  ...
  useEffect(() => {
    fetch("/api/manga").then((res) => res.json()).then((data) => {
      const items = Array.isArray(data.data) ? data.data.filter((m: Manga) => m.genres?.some((g) => g.name === "Action")).sort((a: Manga, b: Manga) => a.rank - b.rank).slice(0, 6) : []
      setMangas(items)
  ...
  return <div className="w-full">
    <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 md:gap-x-6 gap-y-10">
      {mangas.map((manga, index) => <MangaEditorialCard key={manga.mal_id} title={manga.title} image={manga.images?.jpg?.large_image_url} rank={index + 1} href={`/card/${generateSlug(manga.title)}`} eyebrow="Action / Sélection" />)}
    </div>
    <div className="mt-10 text-right"><Link href="/tri" className="kami-link text-[10px] uppercase tracking-[.25em]">Toute la sélection →</Link></div>
  </div>
}
